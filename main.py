...
Помогу тебе сохранить твои проекты и информацию о них! Используй /info, чтобы узнать еоманды.) 
""")
...
1➕/new_project - используй для добавления нового проекта
2🧠💡/skills - использую для назначения навыков для проектов
3📊/projects - используй для отображения всех проектов
4🗑️/delete - используй для удаления проектов
5🔄/update_projects - используй для обновления проекта
...
def delete_handler(message):
    user_id = message.from_user.id
    projects = manager.get_projects(user_id)
    bot.send_message(message.chat_id, 'напиши имя проекта, который хочешь удалить')
    if projects:
        text = "\n".join([f"Project name:{x[2]} \nLink:{x[4]}\n" for x in projects])
        projects = [x[2] for x in projects]
...
def text_handler(message):
    ...
    if project in projects:
        ...
        return
    bot.reply_to(message, "Тебе нужна помощь? Если да, то используй /info")
