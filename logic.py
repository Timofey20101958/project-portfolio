    ...
    def create_tables(self):
        ...
        with con:
            con.execute('''CREATE TABLE projects(
                        project_id INTEGER PRIMARI KEY,
                        user_id INTEGER,
        ...
        con.execute('''CREATE TABLE status(
                    status_id INTEGER PRRIMARI KEY,
                    status_name TEXT
                    )
        ''')
        con.execute('''CREATE TABLE skills(
                    skills_id INTEGER PRIMARI KEY,
                    skill_name TEXT)
        ''')
        con.execute('''CREATE TABLE project_skills(
                    project_id INTEGER,
                    FOREIGN KEY (project_id) REFERENCES skills(skill_id),
...
if __name__ == '__main__':
    ...
    #manager.insert_project([(1, "экологический бот", 'https://github.com/IlyaGolovin01/eco-bot', 5)])
    manager.update_projects('description', ('привет, это описание', "экологический бот", 1))